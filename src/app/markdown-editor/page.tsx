...
export default function MarkdownEditorPage() {
  ...
  const html = useMemo(() => DOMPurify.sanitize(converter.makeHtml(markdown), { USE_PROFILES: { html: true } }), [markdown, converter]);
  return <LocalToolLayout title='Markdown Editor' description='Edit Markdown locally with a live preview. Rendered HTML is sanitized with DOMPurify before it reaches the page.'>
    <section className='mx-auto grid max-w-7xl gap-4 md:grid-cols-2'>
      <div className='flex min-h-[60vh] flex-col'><h2 className='rounded-t border bg-slate-100 p-2 font-semibold'>Markdown</h2><textarea aria-label='Markdown input' value={markdown} onChange={e => setMarkdown(e.target.value)} className='min-h-0 flex-1 rounded-b border p-4 font-mono'/></div>
